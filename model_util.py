...
keyboards = ['keyboard_1', 'keyboard_2', 'keyboard_3', 'keyboard_4']
class EventHandler():
	...
	def draw(self, ax, keyboard_option, origin_x=0, origin_y=0):
		...
		for child in self.children.values():
			if child.name in set(keyboards).difference([keyboards[keyboard_option-1]]):
				self.printSwitch = False
			elif self.printSwitch is False and child.name == keyboards[keyboard_option-1]:
				self.printSwitch = True
			if self.printSwitch:
				
				child.draw(ax, keyboard_option, origin_x + self.top_left_x, origin_y + self.top_left_y)
		self.printSwitch = True
